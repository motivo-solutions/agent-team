...
@test "install does not place deprecated mailbox communication prompt fragments" {
    bash "${INSTALL}" --project-dir "${TEST_PROJECT_DIR}"
    [ ! -f "${TEST_PROJECT_DIR}/.claude/prompts/mailbox-communication.md" ]
    [ ! -f "${TEST_PROJECT_DIR}/.codex/prompts/mailbox-communication.md" ]
}

# シナリオ: 廃止済み skill wrapper を install で再配置しない。
# 保証: legacy な mailbox-send skill ディレクトリは生成されない。
@test "install does not place deprecated mailbox-send skill wrappers" {
    bash "${INSTALL}" --project-dir "${TEST_PROJECT_DIR}"
...
@test "install configures Claude and Codex hooks for mailbox auto-reply" {
    ...
    jq -e '.hooks.UserPromptSubmit[]?.hooks[]? | select(.command == "bash .ai-team/scripts/mailbox-hook.sh record-prompt --default-member leader")' "${TEST_PROJECT_DIR}/.claude/settings.json"
    jq -e '.hooks.Stop[]?.hooks[]? | select(.command == "bash .ai-team/scripts/mailbox-hook.sh flush-reply --default-member leader")' "${TEST_PROJECT_DIR}/.claude/settings.json"
