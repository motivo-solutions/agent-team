...
# 保証: AI_TEAM_MEMBER で指定した member の outbox に response メッセージが生成される。
@test "claude mailbox hook relays last assistant message back to sender" {
    cd "${TEST_PROJECT_DIR}"

    run env AI_TEAM_MEMBER=alpha bash .ai-team/scripts/mailbox-hook.sh record-prompt <<'EOF'
{"prompt":"[Mailbox]\nFrom: bravo\nType: request\n\nPlease review the patch."}
EOF
    [ "$status" -eq 0 ]
    [ -f "${TEST_PROJECT_DIR}/.ai-team/test-session/hook-state/alpha.json" ]

    run env AI_TEAM_MEMBER=alpha bash .ai-team/scripts/mailbox-hook.sh flush-reply <<'EOF'
